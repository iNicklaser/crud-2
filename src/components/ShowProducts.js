...
const ShowProducts = () => {
    ...
    const validar = () => {
        ...
        if(name.trim() === "") {
            ...
            if(operation === 1) {
                ...
                metodo = "POST";
                show_alerta("Producto registrado con éxito","success");
            } 
            else {
                parametros = {id:id, name:name.trim(), description:description.trim(), price:price};
                metodo = "PUT";
                show_alerta("Producto editado con éxito","success");
            }
    ...
    const enviarSolicitud = async(metodo, parametros) => {
        let requestUrl = url;
        if ((metodo === "DELETE" || metodo === "PUT") && parametros.id) {
            requestUrl = url + parametros.id;
        }
    ...
    const deleteProduct = async(id,name) => {
        ...
        MySwal.fire({
            ...
            icon: "question",text: "No podrás revertir esto",
                            ...
                            <tbody className='table-group-divider'>
                                {(products || []).map( (product,i) => (
                                    <tr key={product.id}>
                                        ...
                                        <td>${new Intl.NumberFormat("es-uy").format(product.price)}</td>
                                        <td>
                                            <button onClick={ () => openModal(2, product.id, product.name, product.description, product.price) } 
                                            className='btn btn-warning btn-dark' data-bs-toggle="modal" data-bs-target="#modalProducts">
                ...
                <div className='modal-content'>
                    ...
                    <div className='modal-body'>
                        ...
                        <div className='d-grid col-6 mx-auto'>
                            <button onClick={()=> validar()} className='btn btn-success btn-dark'>
                                <i className='fa-solid fa-floppy-disk'></i> Guardar
                            </button>
                        </div>
                    </div>
                    <div className='modal-footer'>
                        <button id="btnCerrar" className='btn btn-secondary' data-bs-dismiss="modal"> Cerrar</button>
                    </div>
                </div>
            </div>
        </div>
